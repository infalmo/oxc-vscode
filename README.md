...
| `oxc.cmd.oxlint`            | `npx --no-install oxlint --lsp` | `<string>`              | Command to run the oxlint LSP server. Examples: `npx --no-install oxlint --lsp`, `bunx oxlint --lsp`, `/usr/local/bin/oxlint --lsp`.                                                                                                          |
| `oxc.cmd.oxfmt`             | `npx --no-install oxfmt --lsp`  | `<string>`              | Command to run the oxfmt LSP server. Examples: `npx --no-install oxfmt --lsp`, `bunx oxfmt --lsp`, `/usr/local/bin/oxfmt --lsp`.                                                                                                              |
| `oxc.path.tsgolint`         | -             | `<string>`                       | Path to tsgolint binary. Passed as `OXLINT_TSGOLINT_PATH` to oxlint. When unset, oxlint auto-detects from `node_modules/.bin` and system PATH.                                                                                                                                    |
